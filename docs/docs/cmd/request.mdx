...
In interactive terminal sessions, responses from Microsoft Graph mail message endpoints are filtered using the local sender whitelist file `~/.cli-m365-mail-sender-whitelist.json`.

Use [outlook message whitelist](./outlook/message/message-whitelist.mdx) to review blocked messages and add either a sender domain or a specific sender address to the whitelist.
